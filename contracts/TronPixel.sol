...
contract TronPixel {
  ...
  function buyPixels(uint16[] row, uint16[] col, uint24[] newColor) payable {
    uint pLength = row.length;
    require(pLength == col.length);
    ...
    for(uint i = 0; i < pLength ; i++){
        require(row[i] <= size && col[i] <= size);
        key = getKey(row[i], col[i]);
        owner = getPixelOwner(row[i], col[i]);
        price = getPixelPrice(row[i], col[i]);
        SafeMath.add(price,totalPrice);

        money = SafeMath.sub(money,price);
        fee = SafeMath.div(price, feeRatio);
        payout = SafeMath.sub(price, fee);
        users[admin].pendingWithdrawal = SafeMath.add(users[admin].pendingWithdrawal, fee);
        users[owner].pendingWithdrawal = SafeMath.add(users[owner].pendingWithdrawal, payout);
        users[owner].totalSales = SafeMath.add(users[owner].totalSales, payout);

        increase = SafeMath.div(SafeMath.mul(price, incrementRate), 100);
        pixels[key].price = SafeMath.add(price, increase);
        pixels[key].owner = msg.sender;
        PixelTransfer(row[i], col[i], price, owner, msg.sender);
        setPixelColor(row[i], col[i], newColor[i]);
    }
